...
for t in temps:
    ...
    for f in files:
        if os.path.exists(f"cobalt-text-txt/{f}"):
            for i in range(1, 13):
                subprocess.run(f"python llama_cpp_ctsr.py -f cobalt-text-txt/{f} --dir 32b-temp-{t}/{f[:-4]} --ctsr prompts/cats/cat{i}.txt --instruction prompts/ctsr-individual.txt --sys prompts/system-prompt.txt --temp {t}", shell=True,)
        else:
            print(f"{f} not found")
