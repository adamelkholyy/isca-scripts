...
# TODO
# pprint cli args -h and include in README
...
if not args.n_gpu_layers:
    args.n_gpu_layers = next(
        ...
        # Set gpu layers to max if model in lookup dict, else 0 
        (layers for model, layers in gpu_layers_lookup.items() if model == args.model), 0
    )
...
# Get user defined llama.cpp parameters
llama_params = {}

# Iterate through all possible llama parameter args
for arg in llama_parser._group_actions:
    param_name = arg.dest

    # If user passed a value for parameter
    if (param_value := args.__dict__[param_name]):

        # Add to llama parameter dictionary
        llama_params[param_name] = param_value
